...
- (CFHTTPMessageRef)prepareDefaultResponse:(UInt64)contentLength;
@end
...
- (void)dealloc
{
	...
	if(response__) CFRelease(response__);
...
- (void)replyToHTTPRequest
{
	...
	{
		// Status Code 200 - OK
		response = (CFHTTPMessageRef)[self prepareDefaultResponse:contentLength];
    ...
    // security cross-browser issue
	CFHTTPMessageSetHeaderFieldValue(response, CFSTR("Access-Control-Allow-Origin"), CFSTR("*"));
    
	// If they issue a 'HEAD' command, we don't have to include the file
	// If they issue a 'GET' command, we need to include the file
	if([method isEqual:@"HEAD"]) //|| [method isEqual:@"POST"])
	{
...
- (CFHTTPMessageRef)prepareUniRangeResponse:(UInt64)contentLength
{
	...
	response__ = CFHTTPMessageCreateResponse(kCFAllocatorDefault, 206, NULL, kCFHTTPVersion1_1);
	
	DDRange range = [[ranges objectAtIndex:0] ddrangeValue];
	
	NSString *contentLengthStr = [NSString stringWithFormat:@"%qu", range.length];
	CFHTTPMessageSetHeaderFieldValue(response__, CFSTR("Content-Length"), (CFStringRef)contentLengthStr);
	...
	CFHTTPMessageSetHeaderFieldValue(response__, CFSTR("Content-Range"), (CFStringRef)contentRangeStr);
...
- (CFHTTPMessageRef)prepareDefaultResponse:(UInt64)contentLength
{
	...
	// mem leak fix
	//CFHTTPMessageRef response = CFHTTPMessageCreateResponse(kCFAllocatorDefault, 206, NULL, kCFHTTPVersion1_1);
	response__ = CFHTTPMessageCreateResponse(kCFAllocatorDefault, 200, NULL, kCFHTTPVersion1_1);
...
- (CFHTTPMessageRef)prepareMultiRangeResponse:(UInt64)contentLength
{
	...
	CFHTTPMessageSetHeaderFieldValue(response__, CFSTR("Content-Type"), (CFStringRef)contentTypeStr);
...
- (NSData *)preprocessResponse:(CFHTTPMessageRef)response
{
	...
	NSData *result = [(NSData *)CFHTTPMessageCopySerializedMessage(response) autorelease];
