...
- (BOOL)start:(NSError **)error
{
	...
	NSLog(@" success status: %@", (success ? @"yes" : @"no" ));
	
	if(success)
	{
		// Update our port number
		[self setPort:[asyncSocket localPort]];
		NSLog(@" localPort: %i", [asyncSocket localPort]);
		
		// Output console message for debugging purposes
		NSLog(@"Started HTTP server on port %i", port);
		...
		if(type != nil)
		{
			...
			NSLog(@" net service published: %@", netService);
			
			// Do not set the txtRecordDictionary prior to publishing!!!
			// This will cause the OS to crash!!!
			
			NSLog(@" txt record dictionary: %@", txtRecordDictionary);
