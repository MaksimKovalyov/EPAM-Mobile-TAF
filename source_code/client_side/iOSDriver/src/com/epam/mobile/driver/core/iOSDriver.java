...
import com.epam.mobile.driver.guard.controllability.ElementExistAfterTimeoutAssertionError;
import com.epam.mobile.driver.guard.controllability.Guard;
 ...
 * 
 * Note:
 * It is made a little Java Convention Exception for support original idea of driver.
 */
public class iOSDriver implements NativeDriver{

	private static final Logger logger = Logger.getLogger(iOSDriver.class);//TAFLoggerFactory.getLogger(iOSDriver.class); 
    ...
    public iOSDriver(String appURL) {
    ...
    public static void initLogger(Appender appender) {
	...
	public String getElementValue(String classNameChain, 
		...
		//guard controls Exceptional and Assertional situations
		guard.inspect(result);
	...
	public boolean setElementValue(String classNameChain,
		...
		try {
			...
			command.setUp(classNameChain, value);
			result = command.execute();
			
			if (result){
				cmdResult = command.getCommandResult();
			}
		} catch (Exception e) {
			logger.error(e.toString()); clear();
		}
		
		guard.inspect(cmdResult);
	...
	public String doAction(String classNameChain, 
		...
		guard.inspect(result, new ElementExistAfterTimeoutAssertionError("The element doesn't exist after timeout."));
	...
	public boolean waitForElementPresent(String ui_xpath, double time_period){
		...
		return parseBoolean(doAction(ui_xpath, Constants.WAIT, time_period));		
	}
	...
	// Testing of main functions	
		public static void main(String[] args){		
			...
			// smoke tests: driver
			NativeDriver driver = new iOSDriver();
